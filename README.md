...
- **智能提问检测**：内置 16 个常见提问触发词（如"问个问题""请一位同学""怎么看"），命中后提示用户确认，截取从触发词到提问结束的完整内容发送给 AI，确保回答精准对应问题。
...
|  +------------------+  |  +--------------------+  |
+--------------------------------------------------+
|  状态：检测到提问「请问」，请按「结束提问」发送给 AI    |
+--------------------------------------------------+
```

| 区域 | 说明 |
|------|------|
| **实时字幕** | 显示语音识别结果，随老师说话实时滚动。触发提问后，从触发词开始的文本标红。 |
| **AI 回答** | 用户确认提问后，显示 AI 生成的简短回答，带时间戳。 |
| **结束提问** | 默认禁用。检测到提问后变亮可用，点击后将完整提问发送给 AI。 |
| **状态栏** | 显示当前运行状态；触发提问后提示操作。 |
...
│   │   └── http_asr.py      # 兼容 HTTP 分段语音识别
│   ├── detector/
│   │   ├── buffer.py        # 文本滑动窗口
│   │   └── trigger.py       # 提问检测器（手动确认模式）
│   ├── llm/
