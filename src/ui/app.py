...
class SettingsDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Label(self.frm_xunfei, text="APPID:").grid(row=0, column=0, sticky="e", **pad)
        self.entry_app_id = tk.Entry(self.frm_xunfei, width=40)
        self.entry_app_id.grid(row=0, column=1, **pad)
        tk.Label(self.frm_xunfei, text="APIKey:").grid(row=1, column=0, sticky="e", **pad)
        self.entry_api_key = tk.Entry(self.frm_xunfei, width=40)
        self.entry_api_key.grid(row=1, column=1, **pad)
        tk.Label(self.frm_xunfei, text="APISecret:").grid(row=2, column=0, sticky="e", **pad)
        ...
        tk.Label(self.frm_http, text="API Base:").grid(row=0, column=0, sticky="e", **pad)
        self.entry_asr_base = tk.Entry(self.frm_http, width=40)
        self.entry_asr_base.grid(row=0, column=1, **pad)
        tk.Label(self.frm_http, text="API Key:").grid(row=1, column=0, sticky="e", **pad)
        ...
        tk.Label(self.frm_http, text="Language:").grid(row=4, column=0, sticky="e", **pad)
        self.entry_asr_lang = tk.Entry(self.frm_http, width=40)
        self.entry_asr_lang.grid(row=4, column=1, **pad)
        tk.Label(self.frm_http, text="分段时长(ms):").grid(row=5, column=0, sticky="e", **pad)
        ...
        tk.Button(frm_btn, text="保存", command=self._save, width=10).pack(side="right", padx=5)
        tk.Button(frm_btn, text="取消", command=self.destroy, width=10).pack(side="right", padx=5)

    def _on_provider_change(self, event=None):
        provider = self.cmb_provider.get()
        if provider == "xunfei":
            self.frm_xunfei.pack(fill="x", padx=10, pady=5, after=self.cmb_provider.master)
            self.frm_http.pack_forget()
        else:
            self.frm_http.pack(fill="x", padx=10, pady=5, after=self.cmb_provider.master)
    ...
    def _adjust_size_to_content(self):
        ...
        required_height = min(max(620, self.winfo_reqheight() + 20), self.winfo_screenheight() - 80)
    ...
    def _load_values(self):
        ...
        self.entry_asr_base.insert(0, self.config.get("asr.api_base", "https://api.openai.com/v1"))
        ...
        self.entry_asr_seg.insert(0, str(self.config.get("asr.segment_duration_ms", 1500)))

        self.entry_api_base.insert(0, self.config.get("llm.api_base", "https://api.openai.com/v1"))
    ...
    def _save(self):
        ...
        new_config = {
            "asr": {
                ...
                "api_key": self.entry_api_key.get().strip() if provider == "xunfei" else self.entry_asr_key.get().strip(),
...
class MainWindow:
    ...
    def __init__(self, on_start, on_stop, on_settings):
    ...
    def _build_ui(self):
        ...
        tk.Label(toolbar, text="AI课堂助手", font=("Microsoft YaHei", 14, "bold")).pack(side="left")

        self.btn_settings = tk.Button(toolbar, text="设置", command=self.on_settings, width=8)
        self.btn_settings.pack(side="right", padx=5)

        self.btn_toggle = tk.Button(
            toolbar, text="开始听课", command=self._toggle, width=10, bg="#4CAF50", fg="white"
        )
        ...
        self.answer_box.tag_config("meta", foreground="#999999", font=("Microsoft YaHei", 9))
    ...
    def append_subtitle(self, text: str):
        ...
        if text.startswith(self._subtitle_last):
            box.insert("end", text[len(self._subtitle_last):], "teacher")
            ...
            if self._subtitle_last and len(text) < len(self._subtitle_last) * 0.3:
                ...
                if old_tail > 0 and content.endswith(self._subtitle_last):
                    ...
                    box.insert("end", text[common:], "teacher")
                else:
                    # 兜底：加空格追加（极少发生）
                    box.insert("end", " " + text, "teacher")
                self._subtitle_last = text
